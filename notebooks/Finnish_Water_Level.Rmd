...
for (i in seq(1:number_of_cities)){
  ...
  s = seq(1,49)
  s_m = matrix(s,nrow = 49, ncol = 1)
  f_t_ma = matrix(fit_separate_data$beta[,i], nrow = 1, ncol = 4000)
  
  mu = fit_separate_data$alpha[,i] + (s_m %*% f_t_ma)
  mu_matrix = matrix(mu, nrow = 4000, ncol = 49)
  
  post_quan = colQuantiles(mu_matrix, probs=c(0.05, 0.50, 0.95))
  post_quan = unname(post_quan)
  
  df_post = data.frame(quan_05=post_quan[,1], quan_50=post_quan[,2], quan_95=post_quan[,3])

  print(ggplot(df_post, aes(x=x_temp[a:b], y=y_temp[a:b])) +
      geom_point(color="darkblue") +
      geom_line(aes(y = quan_05), color = "darkred", linetype="twodash") +
      geom_line(aes(y = quan_50), color="darkred") +
      geom_line(aes(y = quan_95), color="darkred", linetype="twodash")
  )
}
...
for (i in seq(1:number_of_cities)){
  ...
  s = seq(1,49)
  s_m = matrix(s,nrow = 49, ncol = 1)
  f_t_ma = matrix(fit_hierarchical_data$beta[,i], nrow = 1, ncol = 4000)
  
  mu = fit_hierarchical_data$alpha[,i] + (s_m %*% f_t_ma)
  mu_matrix = matrix(mu, nrow = 4000, ncol = 49)
