...
\begin{figure}[h]
\centerline{\includegraphics[width=150pt]{../img/separatemodel.jpg}}
...
Model convergences means that the sampling chains have merged so that the samples start represent the sampled distribution. The  R-hat  values tells us how well the model has convergenced with respect to a parameter. It generally should be 0.9 and 1.1. It can be seen from Figure #, the  R-hat  values are all very close to 1 and therefore it can be said that the separate model has converged.
...
It can be seen from the values and plot below that almost all k values are less than 0.5, and very few are above 0.5, so these values are biased. Although it is still a reliable model. Further hierarchical model is also calculated for comparison.
...
Model convergences means that the sampling chains have merged so that the samples start represent the sampled distribution. The  R-hat  values tells us how well the model has convergenced with respect to a parameter. It generally should be 0.9 and 1.1. It can be seen from Figure #, the  R-hat  values are all very close to 1 and therefore it can be said that the hierarchical model has converged.
...
Hierarchical model better than Separate model for the following reasons:

1. K-hat values in hierarchical model are very reliable
2. ELPD value is more
3. Effective number of parameters p_loo is less
...
The fact that the sea level rise is not a huge factor in Finland does not, of course, mean that the climate change does not affect Finland and it cna be ignored.

The summary of the results for various cities are shown in figure #. 
...
According to NASA, the average rise in water-level is 3.3 mm. Considering it, we can conclude Figure #.
